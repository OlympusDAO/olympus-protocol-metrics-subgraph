...
export class BophadesModule extends Entity {
  ...
  save(): void {
    const id = this.get("id");
    assert(id != null, "Cannot save BophadesModule entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type BophadesModule must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("BophadesModule", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): BophadesModule | null {
    return changetype<BophadesModule | null>(
      store.get_in_block("BophadesModule", id.toHexString())
    );
  }

  static load(id: Bytes): BophadesModule | null {
    return changetype<BophadesModule | null>(
      store.get("BophadesModule", id.toHexString())
    );
  }
  ...
  set keycode(value: string) {
    this.set("keycode", Value.fromString(value));
  }

  get moduleAddress(): Bytes {
    const value = this.get("moduleAddress");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBytes();
