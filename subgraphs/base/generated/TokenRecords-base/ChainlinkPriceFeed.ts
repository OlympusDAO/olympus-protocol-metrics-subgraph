...
export class ChainlinkPriceFeed extends ethereum.SmartContract {
  ...
  aggregator(): Address {
    const result = super.call("aggregator", "aggregator():(address)", []);

    return result[0].toAddress();
  }

  try_aggregator(): ethereum.CallResult<Address> {
    const result = super.tryCall("aggregator", "aggregator():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    const value = result.value;
  ...
  phaseId(): i32 {
    const result = super.call("phaseId", "phaseId():(uint16)", []);

    return result[0].toI32();
  }

  try_phaseId(): ethereum.CallResult<i32> {
    const result = super.tryCall("phaseId", "phaseId():(uint16)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    const value = result.value;
