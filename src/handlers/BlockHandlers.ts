...
async function processSnapshot(
  ...
  await withContractReadCache(async () => {
    ...
    if (config.chainId === 42161) {
      await pushArbitrumLendingSupply(context, config, client, supplies, timestamp, blockNumber);
    }
  });
...
async function pushOwnedLiquidityRecords(
  ...
  for (const handler of config.ownedLiquidityHandlers) {
    ...
    const includedValue = await getTotalValue(config, context, client, handler, [config.ohmToken], blockNumber);
